...
export function getNextSteps(id) {
  ...
  const request = axios.get(`${ROOT_URL}/nextsteps?engagementId=${id}`, {withCredentials: true});
...
export function createNextSteps(payload) {
  ...
  const request = axios.post(`${ROOT_URL}/nextsteps`, payload, {withCredentials: true});
